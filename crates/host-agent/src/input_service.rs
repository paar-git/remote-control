...
use rc_protocol::{InputAck, InputEvent};
...
impl<S: InputSink> InputService<S> {
    ...
    pub fn new(writer: ChannelWriter, session: Session, sink: S, enabled: bool) -> Self {
        ...
        let input = InputSession::new(sink, HostOs::current(), true);
        Self {
            writer,
            session,
            input,
        ...
        loop {
            tokio::select! {
                ...
                event = reader.next_message::<InputEvent>() => {
                    match event {
                        Ok(Some(event)) => {
                            if let Some(ack) = self.handle(event) {
                                if self.writer.send(&ack).await.is_err() {
                                    break;
                                }
                            }
                        }
                ...
                _ = watermark_timer.tick() => {
                    ...
                    if current != reported {
                        reported = current;
                        if self.writer
                            .send(&InputAck::Applied { watermark: current })
                            .await
                            .is_err()
                        {
                            break;
...
mod tests {
    ...
    impl Harness {
        fn new(permissions: PermissionSet, os: HostOs, enabled: bool) -> Self {
            Self {
                ...
                input: InputSession::new(MockSink::new(), Some(os), true),
                enabled,
            }
        }
    ...
    fn refused_motion_is_not_acknowledged_and_does_not_advance_the_watermark() {
        ...
        // the pointer.
        let mut h = Harness::new(PermissionSet::NONE, HostOs::Windows, true);
        assert_eq!(h.handle(InputEvent::MouseMove { x: 0.5, y: 0.5, seq: 1 }), None);
    ...
    fn permitted_motion_advances_the_watermark_without_an_ack() {
        let mut h = Harness::new(controlling(), HostOs::Windows, true);
        assert_eq!(h.handle(InputEvent::MouseMove { x: 0.5, y: 0.5, seq: 6 }), None);
