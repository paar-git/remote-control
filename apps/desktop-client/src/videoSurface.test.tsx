...
describe('VideoSurface', () => {
  ...
  it('sizes the canvas to the stream the agent actually started', async () => {
    ...
    render(<VideoSurface displayIndex={0} fitted capturing={false} passthrough={false} onPointerSample={() => null} />);
  ...
  it('says the stream failed rather than showing an empty black rectangle', async () => {
    ...
    render(<VideoSurface displayIndex={9} fitted capturing={false} passthrough={false} onPointerSample={() => null} />);
...
describe('VideoSurface input capture', () => {
  ...
  async function focusedSurface(): Promise<HTMLCanvasElement> {
    render(<VideoSurface displayIndex={0} fitted capturing passthrough={false} onPointerSample={() => null} />);
    const canvas = await screen.findByTestId<HTMLCanvasElement>('video-surface');
    canvas.focus();
    return canvas;
  }
  ...
  it('releases a key still held when the session unmounts mid-chord', async () => {
    ...
    const { unmount } = render(<VideoSurface displayIndex={0} fitted capturing passthrough={false} onPointerSample={() => null} />);
    const canvas = await screen.findByTestId<HTMLCanvasElement>('video-surface');
    canvas.focus();
